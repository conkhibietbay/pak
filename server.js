...
const dbReady = Boolean(dbConfig);

const pool = dbReady ? new Pool({
  ...dbConfig,
...
function requireDb(_req, res, next) {
  ...
  return res.status(503).json({
    error: "Database chưa được cấu hình trên Render.",
...
app.get("/health", (_req, res) => {
  res.status(200).json({
    ok: true,
    service: "pak-admin",
    database: dbReady ? "configured" : "not_configured",
    mode: dbReady ? "ready" : "setup_required"
  });
});
...
async function start() {
  try {
    if (dbReady) {
      if (!ADMIN_PASSWORD) throw new Error("ADMIN_PASSWORD is missing. Set it in Render Environment.");
      await initDb();
    } else {
      console.warn("[PAK][SETUP] DATABASE_URL is missing; server is running in setup mode.");
    }
    const server = app.listen(PORT, HOST, () => {
      console.log(`[PAK] listening on http://${HOST}:${PORT}`);
      console.log(`[PAK] database: ${dbReady ? "configured" : "NOT CONFIGURED"}`);
    ...
    console.error("[PAK][HELP] Add DATABASE_URL from Render Postgres and redeploy to enable Account/Key APIs.");
    process.exitCode = 1;
  }
}
start();

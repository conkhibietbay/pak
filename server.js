...
function resolveDatabaseConfig() {
  ...
  throw new Error(
    "DATABASE CONFIG MISSING: set DATABASE_URL (recommended on Render) or PGHOST/PGDATABASE/PGUSER/PGPASSWORD"
  );
}
function assertStartupConfig() {
  if (!ADMIN_PASSWORD) throw new Error("ADMIN_PASSWORD is missing. Add it to Render Environment Variables.");
  return resolveDatabaseConfig();
}

const isProd = process.env.NODE_ENV === "production";
const dbConfig = assertStartupConfig();
const pool = new Pool({
  ...dbConfig,
...
async function start() {
  try {
    ...
    console.error("[PAK][HELP] Render: set DATABASE_URL from the Render Postgres Connect tab and set ADMIN_PASSWORD in Environment.");
    process.exitCode = 1;
  }
}
start();
